...
def find_differing_shared_ids(card_ids, reader_ids):
    """ Find differening shared_ids from two sets """
    card_ids, reader_ids = set(card_ids), set(reader_ids)
    ignore_ids = set([])
    if card_ids != reader_ids:
        common_ids = card_ids.intersection(reader_ids)
        ignore_ids = card_ids - common_ids
        ignore_ids.update(reader_ids - common_ids)
    return ignore_ids
...
def compute_avg_threshold(x1, y1, x2, y2, a=False):
    ...
    if np.any(x2>500):
        ...
        xnew2 = np.linspace(x2.min(), x2.max())
        y2_in = spline(x2, y2, xnew2)
        x1 = xnew2
        y1 = np.repeat(y1[0], len(y2_in))
        y1_in = y1
    if len(x1) == 1 and len(x2) > 1:
        # Draw straight line for x1,y1
        xnew1 = np.linspace(x1.min(), x2.max(), num=len(x2))
        y1 = np.repeat(y1[0], len(y2))
        y1_in = y1
